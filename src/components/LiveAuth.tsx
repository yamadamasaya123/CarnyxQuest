    ...
    try {
      if (isLogin) {
        ...
        if (!email || !displayName || !password) {
          setError("All ritual coordinates must be filled.");
          setLoading(false);
          return;
        }
        if (displayName.length < 3) {
          setError("Tribe moniker must be at least 3 runes long.");
          setLoading(false);
          return;
        }
        ...
        if (res.success) {
          ...
          if (res.error === "ACCOUNT_EXISTS") {
            setError("Tribe member email is already registered here! Redirecting you to login...");
            setTimeout(() => {
              setIsLogin(true);
              setError("Please enter your password to convoke your profile.");
            }, 2500);
          } else {
            setError(res.error || "Failed to establish tribe registration.");
          }
        }
      }
    } catch (err: any) {
      setError(err?.message || "Portal communication failure.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-slate-950 text-slate-100 rounded-2xl p-6 md:p-8 border border-amber-900/40 shadow-2xl relative overflow-hidden max-w-xl mx-auto my-4">
      ...
      <div className="text-center space-y-3 relative z-10">
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-orange-500 to-red-500 font-sans">
          {isLogin ? "CONVOKE PRIMAL ID" : "FORGE TRIBAL PATH"}
        </h2>
        <p className="text-xs text-amber-200/60 max-w-md mx-auto">
          {isLogin
            ...
            : "Select your specialized hunter role, establish coordinates, and ignite your fasting embers."}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5 relative z-10">
          ...
          {!isLogin && (
            <div className="pt-2">
              <label className="block text-[11px] uppercase tracking-wider font-bold mb-3 text-amber-400/80 font-mono">
                Your Hunting Specialization Path
              </label>
              <div className="bg-slate-900/60 border border-amber-900/45 rounded-xl p-4 space-y-3 relative overflow-hidden text-left">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-extrabold text-sm text-amber-200">Chieftain (Starter Class)</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1.5 font-mono text-[10px]">
                  <div className="bg-slate-950/60 border border-slate-900/60 p-2.5 rounded-lg flex flex-col justify-center">
                    ...
                    <span className="text-slate-500 text-[9px] mt-0.5">Unlocks at Level 20</span>
                  </div>
                  <div className="bg-slate-950/60 border border-slate-900/60 p-2.5 rounded-lg flex flex-col justify-center">
                    <span className="text-red-450 font-extrabold flex items-center gap-1">🧭 SLAYER</span>
                    <span className="text-slate-500 text-[9px] mt-0.5">Unlocks at Level 40</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        >
          {loading ? (
            ...
            <span className="flex items-center gap-2">
              ...
              <span>{isLogin ? "Convoke Profile" : "Ignite Primal Heart"}</span>
            </span>
          )}
        </button>
        ...
        <div className="text-center pt-2">
          ...
          >
            {isLogin ? "No tribal lineage yet? Initiate covenant here" : "Return to Primal Id Gateway"}
          </button>
        </div>
      </form>
    </div>
